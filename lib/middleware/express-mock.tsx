...
export default (req, res, next) => {
  ...
  req.baseUrl = "";
  res.body = false;
  res.location = (url) => {res.set('Location', url); res.status(302);};
  req.is = (consumes) => typeis(req, consumes);
  req.get = (name) => req.headers?.[name];
  req.set = (name, value) => req.headers[name] = value;
  res.set = (name, value) => {console.log([name, value]); res.setHeader(name, value)};
  res.get = (name) => {res.headers?.[name]};
  res.type = (ct) => {res.set('Content-Type', ct);};
...
export const useMiddleware = (swaggerMiddlewares, req, res) => {
  return async.series(swaggerMiddlewares.map(middleware => (resolve: () => void) => {
